...
/*******************************************************************************
* Function Name: Clock_Audio_Start
********************************************************************************
*
* Summary:
...
void Clock_Audio_Start(void) 
{
    ...
    Clock_Audio_CLKEN |= Clock_Audio_CLKEN_MASK;
	Clock_Audio_CLKSTBY |= Clock_Audio_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Audio_Stop
********************************************************************************
*
* Summary:
...
void Clock_Audio_Stop(void) 
{
    ...
    Clock_Audio_CLKEN &= (uint8)(~Clock_Audio_CLKEN_MASK);
	Clock_Audio_CLKSTBY &= (uint8)(~Clock_Audio_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Audio_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Audio_CLKEN & Clock_Audio_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Audio_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Audio_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Audio__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Audio_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Audio_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Audio_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Audio_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Audio_GetSourceRegister();
    uint16 oldDivider = Clock_Audio_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Audio_CLKEN & Clock_Audio_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Audio_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Audio_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Audio_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Audio_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Audio_MOD_SRC |= modeBitMask & (uint8)Clock_Audio_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Audio_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Audio_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Audio_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Audio_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Audio_MOD_SRC & (uint8)(Clock_Audio_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Audio_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Audio_GetDividerRegister();
    uint8 oldSrc = Clock_Audio_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Audio_MOD_SRC |= CYCLK_SSS;
        Clock_Audio_MOD_SRC =
            (Clock_Audio_MOD_SRC & (uint8)(~Clock_Audio_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Audio_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Audio_MOD_SRC & Clock_Audio_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Audio_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Audio_PHASE = clkPhase & Clock_Audio_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Audio_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Audio_PHASE & Clock_Audio_PHASE_MASK;
}
