...
static uint8 XDATA I2C_MPU6050_slReadBuf[I2C_MPU6050_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer - The host reads responses from it */
static uint8 XDATA I2C_MPU6050_slWriteBuf[I2C_MPU6050_BTLDR_SIZEOF_WRITE_BUFFER];
...
*
* Global variables:
*  I2C_MPU6050_slWriteBuf - The global variable used to store a received
*                                command.
*  I2C_MPU6050_slReadBuf -  The global variable used to store a response.
...
void I2C_MPU6050_CyBtldrCommStart(void) CYSMALL 
{
    ...
    I2C_MPU6050_SlaveInitWriteBuf(I2C_MPU6050_slWriteBuf, I2C_MPU6050_BTLDR_SIZEOF_WRITE_BUFFER);
    I2C_MPU6050_SlaveInitReadBuf (I2C_MPU6050_slReadBuf, 0u);
...
*
*******************************************************************************/
cystatus I2C_MPU6050_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
...
*
* Global variables:
*  I2C_MPU6050_slWriteBuf - The global variable used to store a
*                                received command.
*
*******************************************************************************/
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from host */
        {
            ...
            if(0u != (I2C_MPU6050_slStatus & I2C_MPU6050_SSTAT_WR_CMPLT))
            {
                /* Define how many bytes host has been written */
                byteCount = I2C_MPU6050_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) I2C_MPU6050_slWriteBuf,
                              I2C_MPU6050_MIN_UNT16(byteCount, size));
